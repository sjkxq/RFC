package com.sjkxq.client.servicecenter.balance.impl;

import com.sjkxq.client.servicecenter.balance.LoadBalance;
...
public class ConsistencyHashBalance implements LoadBalance {
    ...
    public void init(List<String> serviceList) {
        for (String server : serviceList) {
            ...
            log.debug("真实节点[{}] 被添加", server);
            for (int i = 0; i < VIRTUAL_NUM; i++) {
                String virtualNode = server + "&&VN" + i;
                int hash = getHash(virtualNode);
                shards.put(hash, virtualNode);
                log.debug("虚拟节点[{}] hash:{}，被添加", virtualNode, hash);
            }
        }
    }
